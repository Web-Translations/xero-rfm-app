...
{
    /**
     * Show organisation management page
     */
    public function index(Request $request)
    {
        $user = $request->user();
        $organisations = $user->getAllXeroConnections();
    ...
    public function disconnect(Request $request, int $connectionId)
    {
        ...
        DB::transaction(function () use ($connection, $user) {
            // Delete RFM reports for this organisation
            ...
            // Delete invoices for this organisation
            DB::table('xero_invoices')
                ->where('user_id', $user->id)
                ->where('tenant_id', $connection->tenant_id)
                ->delete();

            // Delete clients for this organisation
            DB::table('clients')
                ->where('user_id', $user->id)
                ->where('tenant_id', $connection->tenant_id)
                ->delete();
